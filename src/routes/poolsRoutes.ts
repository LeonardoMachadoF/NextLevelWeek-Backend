...
export const poolsRoutes = async (fastify: FastifyInstance) => {
    ...
    fastify.post('/pools', async (req, res) => {
        ...
        let { title } = createPoolBody.parse(req.body);
    ...
    fastify.post('/pools/join', { onRequest: [authenticate] }, async (req, res) => {
        ...
        if (pool.participants.length > 0) {
            return res.status(400).send({ message: 'You have already joined this pool.' })
        };
